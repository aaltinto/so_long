...
/*   Updated: 2023/12/20 13:09:33 by aaltinto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_image(t_vars *vars, char c, int x, int y)
{
	...
	i = rand();
	i = i % 2;
	mlx_put_image_to_window(vars->mlx, vars->win, vars->img.back[i], x, y);
	if (c == 'P')
		mlx_put_image_to_window(vars->mlx, vars->win,
			vars->img.player[i], x, y);
	if (c == '1')
		mlx_put_image_to_window(vars->mlx, vars->win, vars->img.wall[i], x, y);
	if (c == 'C')
		mlx_put_image_to_window(vars->mlx, vars->win,
			vars->img.collect[i], x, y);
	if (c == 'E')
